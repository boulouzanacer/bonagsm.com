...
@if($errors->has('database_password') || $errors->has('tables'))
    <div class="mb-4 rounded-2xl border border-red-400/20 bg-red-500/10 px-4 py-3 text-sm text-red-200">
        <ul class="list-disc pl-5 space-y-1">
            @foreach($errors->get('database_password') as $message)
                <li>{{ $message }}</li>
            @endforeach
            @foreach($errors->get('tables') as $message)
                <li>{{ $message }}</li>
            @endforeach
        </ul>
    </div>
...
@if($isAdmin)
    ...
    <div id="database-tools" class="mt-4 rounded-[24px] p-5 border border-red-400/20 bg-gradient-to-br from-red-500/10 via-transparent to-amber-500/10">
        <div class="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
            ...
            <form method="POST"
                  action="{{ url('/fournisseur/dashboard/database/unlock') }}"
        ...
        <form method="POST"
              action="{{ url('/fournisseur/dashboard/database/reset') }}"
              class="mt-5"
