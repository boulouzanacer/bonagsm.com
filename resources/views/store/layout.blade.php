    ...
    <style>
        ...
        .promo-product-card{
            ...
            background:
                ...
                linear-gradient(180deg, rgba(255,255,255,0.99), rgba(240,253,244,0.95));
        }
        .promo-product-card .product-media{
            background:
                ...
                linear-gradient(135deg, #f8fafc, #ecfdf5 70%);
        }
                ...
                <a href="{{ url('/') }}" class="inline-flex items-center gap-4 min-w-0 sm:min-w-[280px]">
                    @if(($storeFrs?->logo_url ?? '') !== '')
                        <img src="{{ $storeFrs->logo_url }}"
                             alt=""
                             class="h-14 w-28 sm:h-20 sm:w-40 rounded-3xl object-contain border border-white/80 p-2 sm:p-2.5 flex-shrink-0 shadow-lg shadow-slate-200/80">
                    @else
