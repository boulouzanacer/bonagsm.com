    ...
    public function index(ProduitIndexRequest $request)
    {
        ...
        $items = $paginator->getCollection()->map(function (Produit $p) use ($client) {
            return [
                ...
                'pv_2' => (float) $p->pv_2,
                'pv_3' => (float) $p->pv_3,
                'tva' => $p->tva === null ? null : (float) $p->tva,
                'promo_enabled' => (bool) ($p->promo_enabled ?? false),
                'promo_start_at' => optional($p->promo_start_at)->toIso8601String(),
                'promo_end_at' => optional($p->normalizedPromotionEnd())->toIso8601String(),
                'promo_quantity' => $p->promoThresholdQuantity(),
                'promo_price' => $p->promo_price === null ? null : (float) $p->promo_price,
                'promo_active_now' => $p->isPromotionActive(),
                'prix_standard' => (float) $p->prixStandardPourQuantite($client instanceof Client ? $client : null, 1),
